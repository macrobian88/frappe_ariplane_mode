...
def create_invoice(doc, method=None):
    ...
    frappe.db.set_value("shop_contract", doc.name, {
        "invoice_generated": 1,
...
def mark_invoice_as_paid(contract_name):
    contract = frappe.get_doc("shop_contract", contract_name)
    ...
    try:
        ...
        frappe.db.set_value("shop_contract", contract_name, "payment_status", "Paid")
        frappe.db.set_value("shop_contract", contract_name, "payment_entry", pe.name)
        frappe.db.commit()
